...
class WallArt extends StatefulWidget {
  const WallArt({Key? key}) : super(key: key);

  @override
  State<WallArt> createState() => _WallArtState();
}

class _WallArtState extends State<WallArt> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectHomeFurniture", isEqualTo: "Wall Art").snapshots().listen((event) {
