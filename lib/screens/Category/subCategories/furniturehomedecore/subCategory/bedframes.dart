...
class BedFrames extends StatefulWidget {
  const BedFrames({Key? key}) : super(key: key);

  @override
  State<BedFrames> createState() => _BedFramesState();
}

class _BedFramesState extends State<BedFrames> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectHomeFurniture", isEqualTo: "Bed Frames").snapshots().listen((event) {
