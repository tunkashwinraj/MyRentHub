...
class CamCoders extends StatefulWidget {
  const CamCoders({Key? key}) : super(key: key);

  @override
  State<CamCoders> createState() => _CamCodersState();
}

class _CamCodersState extends State<CamCoders> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectElectronics", isEqualTo: "Camcoders").snapshots().listen((event) {
