...
class MassageChairs extends StatefulWidget {
  const MassageChairs({Key? key}) : super(key: key);

  @override
  State<MassageChairs> createState() => _MassageChairsState();
}

class _MassageChairsState extends State<MassageChairs> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectHeathFitness", isEqualTo: "Massage Chairs").snapshots().listen((event) {
