...
class DrumSets extends StatefulWidget {
  const DrumSets({Key? key}) : super(key: key);

  @override
  State<DrumSets> createState() => _DrumSetsState();
}

class _DrumSetsState extends State<DrumSets> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectMusicalInstruments", isEqualTo: "Drum Sets").snapshots().listen((event) {
