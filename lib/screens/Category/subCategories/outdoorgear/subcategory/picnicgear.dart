...
class PicnicGear extends StatefulWidget {
  const PicnicGear({Key? key}) : super(key: key);

  @override
  State<PicnicGear> createState() => _PicnicGearState();
}

class _PicnicGearState extends State<PicnicGear> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectSportsEquipment", isEqualTo: "Picnic Gear").snapshots().listen((event) {
