...
class CampingGear extends StatefulWidget {
  const CampingGear({Key? key}) : super(key: key);

  @override
  State<CampingGear> createState() => _CampingGearState();
}

class _CampingGearState extends State<CampingGear> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectTravelLuggage", isEqualTo: "Camping Gear").snapshots().listen((event) {
