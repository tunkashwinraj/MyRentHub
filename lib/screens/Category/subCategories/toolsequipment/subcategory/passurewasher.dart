...
class PassureWasher extends StatefulWidget {
  const PassureWasher({Key? key}) : super(key: key);

  @override
  State<PassureWasher> createState() => _PassureWasherState();
}

class _PassureWasherState extends State<PassureWasher> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectToolsEquipment", isEqualTo: "Passure Washer").snapshots().listen((event) {
