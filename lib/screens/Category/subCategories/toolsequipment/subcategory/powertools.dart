...
class PowerTools extends StatefulWidget {
  const PowerTools({Key? key}) : super(key: key);

  @override
  State<PowerTools> createState() => _PowerToolsState();
}

class _PowerToolsState extends State<PowerTools> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectToolsEquipment", isEqualTo: "Power Tools").snapshots().listen((event) {
