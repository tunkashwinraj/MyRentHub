...
class GardingTools extends StatefulWidget {
  const GardingTools({Key? key}) : super(key: key);

  @override
  State<GardingTools> createState() => _GardingToolsState();
}

class _GardingToolsState extends State<GardingTools> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectToolsEquipment", isEqualTo: "Garding Tools").snapshots().listen((event) {
