...
class AirFaryes extends StatefulWidget {
  const AirFaryes({Key? key}) : super(key: key);

  @override
  State<AirFaryes> createState() => _AirFaryesState();
}

class _AirFaryesState extends State<AirFaryes> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectKitchenAppliences", isEqualTo: "AirFaryes").snapshots().listen((event) {
