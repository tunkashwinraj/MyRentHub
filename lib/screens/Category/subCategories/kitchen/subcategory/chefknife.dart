...
class ChefKnife extends StatefulWidget {
  const ChefKnife({Key? key}) : super(key: key);

  @override
  State<ChefKnife> createState() => _ChefKnifeState();
}

class _ChefKnifeState extends State<ChefKnife> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectKitchenAppliences", isEqualTo: "Chef Knife").snapshots().listen((event) {
