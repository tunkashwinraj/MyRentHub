...
class SlowCookers extends StatefulWidget {
  const SlowCookers({Key? key}) : super(key: key);

  @override
  State<SlowCookers> createState() => _SlowCookersState();
}

class _SlowCookersState extends State<SlowCookers> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectKitchenAppliences", isEqualTo: "Slow Cookers").snapshots().listen((event) {
