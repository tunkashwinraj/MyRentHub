...
class FoodProcessor extends StatefulWidget {
  const FoodProcessor({Key? key}) : super(key: key);

  @override
  State<FoodProcessor> createState() => _FoodProcessorState();
}

class _FoodProcessorState extends State<FoodProcessor> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      ...
      FirebaseFirestore.instance.collection("Categories").where(
          "selectKitchenAppliences", isEqualTo: "Food Processor")
          .snapshots()
          .listen((event) {
        data.clear();
        setState(() {});
