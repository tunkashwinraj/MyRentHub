...
class PlaySeats extends StatefulWidget {
  const PlaySeats({Key? key}) : super(key: key);

  @override
  State<PlaySeats> createState() => _PlaySeatsState();
}

class _PlaySeatsState extends State<PlaySeats> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectBabyKidsGear", isEqualTo: "PlaySeats").snapshots().listen((event) {
