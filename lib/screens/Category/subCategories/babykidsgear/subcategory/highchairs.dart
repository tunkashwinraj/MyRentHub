...
class HighChairs extends StatefulWidget {
  const HighChairs({Key? key}) : super(key: key);

  @override
  State<HighChairs> createState() => _HighChairsState();
}

class _HighChairsState extends State<HighChairs> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectBabyKidsGear", isEqualTo: "High Chairs").snapshots().listen((event) {
