...
class Strollers extends StatefulWidget {
  const Strollers({Key? key}) : super(key: key);

  @override
  State<Strollers> createState() => _StrollersState();
}

class _StrollersState extends State<Strollers> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectBabyKidsGear", isEqualTo: "Strollers").snapshots().listen((event) {
