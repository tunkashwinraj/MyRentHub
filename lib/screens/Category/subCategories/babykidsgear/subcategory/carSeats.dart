...
class CarSeats extends StatefulWidget {
  const CarSeats({Key? key}) : super(key: key);

  @override
  State<CarSeats> createState() => _CarSeatsState();
}

class _CarSeatsState extends State<CarSeats> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectBabyKidsGear", isEqualTo: "CarSeats").snapshots().listen((event) {
