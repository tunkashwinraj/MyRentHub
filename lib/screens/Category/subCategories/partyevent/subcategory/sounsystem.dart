...
class SounSystem extends StatefulWidget {
  const SounSystem({Key? key}) : super(key: key);

  @override
  State<SounSystem> createState() => _SounSystemState();
}

class _SounSystemState extends State<SounSystem> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectPartyEventSupplies", isEqualTo: "SounSystem").snapshots().listen((event) {
