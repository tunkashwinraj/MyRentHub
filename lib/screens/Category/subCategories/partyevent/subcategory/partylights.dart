...
class PartyLights extends StatefulWidget {
  const PartyLights({Key? key}) : super(key: key);

  @override
  State<PartyLights> createState() => _PartyLightsState();
}

class _PartyLightsState extends State<PartyLights> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectPartyEventSupplies", isEqualTo: "Party Lights").snapshots().listen((event) {
