...
class TabletsParty extends StatefulWidget {
  const TabletsParty({Key? key}) : super(key: key);

  @override
  State<TabletsParty> createState() => _TabletsPartyState();
}

class _TabletsPartyState extends State<TabletsParty> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectPartyEventSupplies", isEqualTo: "Tablets").snapshots().listen((event) {
