...
class PartyWaer extends StatefulWidget {
  const PartyWaer({Key? key}) : super(key: key);

  @override
  State<PartyWaer> createState() => _PartyWaerState();
}

class _PartyWaerState extends State<PartyWaer> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectFashionAccerioes", isEqualTo: "Party Waer").snapshots().listen((event) {
