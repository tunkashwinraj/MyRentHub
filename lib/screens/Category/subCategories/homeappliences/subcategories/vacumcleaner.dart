...
class VacumCleaner extends StatefulWidget {
  const VacumCleaner({Key? key}) : super(key: key);

  @override
  State<VacumCleaner> createState() => _VacumCleanerState();
}

class _VacumCleanerState extends State<VacumCleaner> {
  List<DataModel> data = [];
  ...
  getData() {
    try {
      data.clear();
      setState(() {});
      FirebaseFirestore.instance.collection("Categories").where("selectHomeAppliences", isEqualTo: "VacumCleaner").snapshots().listen((event) {
